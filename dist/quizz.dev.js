...
var answerBox = document.querySelector('#answers-box');
var quizzContainer = document.querySelector('#quizz-container');
var scoreContainer = document.querySelector('#score-container');
var letters = ['a', 'b', 'c', 'd', 'e'];
var points = 0;
var actualQuestion = 0; // perguntas
...
function createQuestion(i) {
  ...
  var oldButtons = answerBox.querySelectorAll('button');
  oldButtons.forEach(function (btn) {
    return btn.remove();
  }); // alterar texto da pergunta

  var questionText = question.querySelector('#question-text');
  var questionNumber = question.querySelector('#question-number');
  questionText.textContent = questions[i].question;
  questionNumber.textContent = i + 1; // inserir alternativas

  questions[i].answers.forEach(function (answer, index) {
    var answerTemplate = document.querySelector('.answer-template').cloneNode(true);
    var letterBtn = answerTemplate.querySelector('.btn-letter');
    var answerText = answerTemplate.querySelector('.question-answer');
    letterBtn.textContent = letters[index];
    answerText.textContent = answer.answer;
    answerTemplate.setAttribute('correct-answer', answer.correct); // remover hide e template class

    answerTemplate.classList.remove('hide', 'answer-template'); // inserir alternativa na tela

    answerBox.appendChild(answerTemplate); // inserir evento click no botão
...
function checkAnswer(btn) {
  var buttons = answerBox.querySelectorAll('button');
...
function showSuccessMessage() {
  hideOrShowQuizz();
  var score = (points / questions.length * 100).toFixed(2);
  var displayScore = document.querySelector('#display-score span');
  displayScore.textContent = score;
  var correctAnswers = document.querySelector('#correct-answers');
  correctAnswers.textContent = points;
  var totalQuestions = document.querySelector('#questions-qty');
